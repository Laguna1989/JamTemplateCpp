...
TEST_F(BasicActionCommandsWithGame, CamReset)
{
    ...
    g->actionCommandManager().executeCommand("cam.reset");
}
...
TEST_F(BasicActionCommandsWithGame, CamShake)
{
    ...
    g->actionCommandManager().executeCommand("cam.shake 1 2");
}
...
TEST_F(BasicActionCommandsWithGame, CamMove)
{
    ...
    g->actionCommandManager().executeCommand("cam.move 100 200");
}
...
TEST_F(BasicActionCommandsWithGame, CamPos)
{
    ...
    g->actionCommandManager().executeCommand("cam.pos 20 400");
}
...
TEST_F(BasicActionCommandsWithGame, CamZoom)
{
    ...
    g->actionCommandManager().executeCommand("cam.zoom 2.5");
}
...
{
    addBasicActionCommands(g);
    ASSERT_NO_THROW(g->actionCommandManager().executeCommand("textureManagerInfo"));
}
