...
{
    createOverlayAlphaTween();
    createTitleAlphaTween();
    createCreditsPositionTween();
    createExplanationScaleTween();
}

void StateMenu::createExplanationScaleTween()
{
    auto s2 = m_test_Explanation->getPosition() + jt::Vector2 { -1000, 0 };
    auto e2 = m_test_Explanation->getPosition();

    auto tw2 = jt::TweenPosition<jt::SmartText>::create(m_test_Explanation, 0.5f, s2, e2);
    tw2->setStartDelay(0.3f);
    tw2->setSkipFrames();

    tw2->addCompleteCallback([this]() {
        auto ts1 = jt::TweenScale<jt::SmartText>::create(
...
void StateMenu::createTitleAlphaTween()
{
...
void StateMenu::createOverlayAlphaTween()
{
...
void StateMenu::createCreditsPositionTween()
{
    auto s3 = m_text_Credits->getPosition() + jt::Vector2 { 0, 100 };
    auto e3 = m_text_Credits->getPosition();

    auto tw3 = jt::TweenPosition<jt::SmartText>::create(m_text_Credits, 0.35f, s3, e3);
